...
public class VideoTexture_Lite2 : MonoBehaviour 
{
	public float FPS = 30;
	...
	// Application.targetFrameRate = 60; (Optional for smoother scrubbing on capable systems)
		...
		view = gameObject.GetComponent<RawImage>();
	...
	// Forces audio sync on first play (helpful for some devices)
		...
		index = myAudio.attachedAudioSource.timeSamples / (myAudio.attachedAudioSource.clip.samples / lastFrame);

		intIndex = (int)index;

		//myAudio.attachedAudioSource.timeSamples = (int)((myAudio.attachedAudioSource.clip.samples / lastFrame) * index);
